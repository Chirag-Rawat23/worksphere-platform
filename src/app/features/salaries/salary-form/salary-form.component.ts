...
export class SalaryFormComponent implements OnInit {
  ...
  constructor(
    ...
    const baseAmountControl = this.form.get('baseAmount');
    if (baseAmountControl) {
      baseAmountControl.valueChanges.subscribe(() => this.calculateDerived());
    }
    
    const bonusControl = this.form.get('bonus');
    if (bonusControl) {
      bonusControl.valueChanges.subscribe(() => this.calculateDerived());
    }
  }
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.loadSalary(parseInt(id, 10));
    } else if (employeeId) {
      this.loadEmployee(parseInt(employeeId, 10));
      this.form.patchValue({ employeeId: parseInt(employeeId, 10) });
      
      // Load previous salary if available
      this.salaryService.getSalariesByEmployee(parseInt(employeeId, 10)).subscribe({
        next: (salaries) => {
  ...
  calculateDerived(): void {
    ...
    const baseAmountControl = this.form.get('baseAmount');
    const bonusControl = this.form.get('bonus');
    const baseAmount = baseAmountControl?.value || 0;
    const bonus = bonusControl?.value || 0;
